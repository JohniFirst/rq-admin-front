...
const BaseDraggableTable = <T extends object>({
  ...
  const [dataSource, setDataSource] = useState<T[]>(() =>
    tableProps.dataSource ? [...(tableProps.dataSource as T[])] : [],
  )
  ...
  useEffect(() => {
    if (tableProps.dataSource) {
      setDataSource([...(tableProps.dataSource as T[])])
    }
  ...
  const handleExport = () => {
    ...
    const excelData: ExcelData = {
      ...
      data: ((tableProps.dataSource as T[]) ?? []).map(item => {
        return keys.map(key => (item as T)[key as keyof T])
      }),
    }
