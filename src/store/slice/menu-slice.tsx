...
// 递归将 title 转 label
function convertMenuTitleToLabel(menuList: MenuApiResponse[]): MenuItem[] {
  return menuList.map((item: MenuApiResponse) => {
    const { ...rest } = item
    const newItem: MenuItem = { ...rest, label: item.title, key: item.url }
