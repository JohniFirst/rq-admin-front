...
const MessageNotification: React.FC<MessageNotificationProps> = ({ unreadMessages }) => {
        ...
        <div className="space-y-4">
          <div className="space-y-4 max-h-[400px] overflow-y-auto pr-2">
            {[...Array(3)].map((_, index) => (
              <div
                key={`unread-${index}`}
                className="p-5 bg-gray-50 dark:bg-gray-800 rounded-lg hover:bg-gray-100 dark:hover:bg-gray-700 transition-all duration-300 cursor-pointer"
              >
          ...
          <Button
            ...
            className="w-full bg-indigo-600 hover:bg-indigo-700 border-none shadow-md hover:shadow-lg transition-all duration-300 h-10 text-base"
          >
            全部标记为已读
          </Button>
        </div>
      ...
      children: (
        <div className="space-y-4 max-h-[400px] overflow-y-auto pr-2">
          {[...Array(5)].map((_, index) => (
            <div
              key={`read-${index}`}
              className="p-5 bg-gray-50 dark:bg-gray-800 rounded-lg opacity-60 transition-all duration-300"
            >
              <div className="flex items-start gap-4">
  ...
  return (
    <div>
      <Popover
        title={
          <h3 className="text-xl font-semibold text-gray-800 dark:text-gray-200 mb-4">消息中心</h3>
        }
        trigger="click"
        content={
          <Tabs
      ...
      >
        ...
        >
          <BellOutlined className="text-2xl text-gray-600 dark:text-gray-300 hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors duration-300" />
        </Badge>
      </Popover>
    </div>
  )
